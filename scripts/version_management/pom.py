...
class POM(object):
    ...
    def calculateEffectiveMavenCoordinates(self):
        ...
        if(self.mavenCoordinates != None
            and self.mavenCoordinates.version == None):
            if(self.parentMavenCoordinates != None 
                and self.parentMavenCoordinates.version != None):
                self.effectiveMavenCoordinates.version = self.parentMavenCoordinates.version
            else:
                raise Exception("filename: %s is missing version." % (self.filename, select))
        if(self.mavenCoordinates != None
            and self.mavenCoordinates.groupId == None):
            if(self.parentMavenCoordinates != None
                and self.parentMavenCoordinates.groupId != None):
        ...
        if("${" in str(self.effectiveMavenCoordinates.version)):
            self.effectiveMavenCoordinates.version = self.findPropertiesFromParent(self.effectiveMavenCoordinates.version)
